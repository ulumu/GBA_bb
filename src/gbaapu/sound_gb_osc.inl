...
INLINE void Gba_Osc::update_amp( int32_t time, int new_amp )
{
...
INLINE int Gba_Env::reload_env_timer()
{
   int raw = regs [2] & 7;
   env_delay = (raw ? raw : 8);
   return raw;
...
void Gba_Sweep_Square::calc_sweep( bool update )
{
...
void Gba_Sweep_Square::clock_sweep()
{
        if ( --sweep_delay <= 0 )
        {
                reload_sweep_timer();
...
int Gba_Wave::access( unsigned addr ) const
{
...
int Gba_Osc::write_trig( int frame_phase, int max_len, int old_data )
{
        int data = regs [4];
...
INLINE void Gba_Env::zombie_volume( int old, int data )
{
	int v = volume;
...
bool Gba_Env::write_register( int frame_phase, int reg, int old, int data )
{
...
bool Gba_Square::write_register( int frame_phase, int reg, int old_data, int data )
{
   bool result = Gba_Env::write_register( frame_phase, reg, old_data, data );
...
void Gba_Square::run( int32_t time, int32_t end_time )
{
        /* Calc duty and phase*/
	...
	duty = 8 - duty;
        ...
        int vol = 0;
        Blip_Buffer_Gba* const out = output;
        if ( out )
        {
                int amp = dac_off_amp;
                if ( GB_ENV_DAC_ENABLED() )
...
void Gba_Noise::run( int32_t time, int32_t end_time )
{
        /* Determine what will be generated*/
        int vol = 0;
        Blip_Buffer_Gba* const out = output;
...
void Gba_Wave::run( int32_t time, int32_t end_time )
{
        /* Calc volume*/
