...
static Blip_Buffer_Gba bufs_buffer [BUFS_SIZE];
...
static INLINE void stereo_buffer_mixer_read_pairs( int16_t* out, int count )
{
	...
	Blip_Buffer_Gba* buf = &bufs_buffer [2];
	{
		--buf;
		--outtemp;
...
void soundEvent_u8(uint32_t address, uint8_t data)
{
	int gb_addr = table[address - 0x60];
	ioMem[address] = data;
...
void process_sound_tick_fn (void)
{
	...
	if(SOUND_CLOCK_TICKS > gba_apu.last_time)
		gb_apu_run_until_( SOUND_CLOCK_TICKS );

	gba_apu.frame_time -= SOUND_CLOCK_TICKS;
	gba_apu.last_time -= SOUND_CLOCK_TICKS;
	...
	int const soundBufferLen = ( soundSampleRate / 60 ) * 4;
	...
	while ( stereo_buffer_samples_avail() >= out_buf_size )
	{
		stereo_buffer_read_samples( (int16_t*) soundFinalWave, out_buf_size);

		soundDriver->write((u16 *)soundFinalWave, soundBufferLen);
...
static void apply_muting (void)
{
	...
	{
		gb_apu_set_output( NULL, NULL, NULL, 0 );
		gb_apu_set_output( NULL, NULL, NULL, 1 );
		gb_apu_set_output( NULL, NULL, NULL, 2 );
		gb_apu_set_output( NULL, NULL, NULL, 3 );
	}
}
