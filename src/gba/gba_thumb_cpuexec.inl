...
static  void thumb47(u32 opcode)
{
	...
	if(bus.reg[base].I & 1) {
		...
		if ( (bus.reg[15].I & 0x0F000000) == 0 )
			bus.reg[15].I |= BIOS_VIRTUAL_BASE;
#endif
		bus.armNextPC = bus.reg[15].I;
		bus.reg[15].I += 2;
		THUMB_PREFETCH;
...
static  void thumb48(u32 opcode)
{
	...
	DATATICKS_UPDATE_ON_BUSPREFETCH;
	u32 address = (bus.reg[15].I & 0xFFFFFFFC) + ((opcode & 0xFF) << 2);
	bus.reg[regist].I = CPUReadMemory(address); //CPUReadMemoryQuick(address);
	bus.busPrefetchCount=0;
...
static  void thumb88(u32 opcode)
{
	u32 Rd     = opcode & 7;
	u32 Rs     = (opcode >> 3) & 7;
	u32 Offset = ((opcode >> 5) & 0x3E);

	DATATICKS_UPDATE_ON_BUSPREFETCH;
	u32 address  = bus.reg[Rs].I + Offset;
	int addrpage = (address >> 24) & 0xF;
	bus.reg[Rd].I = CPUReadHalfWord(address);
	int dataticks_value = DATATICKS_ACCESS_16BIT_PAGE(addrpage);
	DATATICKS_ACCESS_BUS_PREFETCH_ARM(addrpage, dataticks_value);
	clockTicks = 3 + dataticks_value + codeTicksAccess(bus.armNextPC, BITS_16);
}
...
#ifdef INSN_COUNTER
static void thumb_countstat(u32 opcode)
{
    static int thumb_insncount = 0;    // number of insns seen
    static int thumb_executed = 0;     // number of insns executed
    static int thumb_mergewith[1024];  // map instructions to routines
    static int thumb_count[1024];      // count of each 12-bit code
    int index = (opcode>>6);
    static FILE *thumb_outfile = NULL;
    int i,j;

    if (!thumb_insncount) {
        for (i = 0; i < 1024; i++) {
            for (j = 0; j < i; j++) {
                if (thumbInsnTable[i] == thumbInsnTable[j])
                    break;
            }
            thumb_mergewith[i] = j;
        }
        ...
        char  fullPath[512];
        char  filename[] = "VBA-thumbcount.txt";
        ...
        snprintf(fullPath, sizeof(fullPath), "%s/../data/%s", homeDir, filename);
        thumb_outfile = fopen(fullPath, "w");

        if (thumb_outfile == NULL)
        {
             snprintf(fullPath, sizeof(fullPath), "%s/../../data/%s", homeDir, filename);
             thumb_outfile = fopen(fullPath, "w");
        }
    ...
    thumb_count[thumb_mergewith[index]]++;
    thumb_executed++;

    thumb_insncount++;
    if (thumb_outfile && thumb_insncount%1000000 == 0) {
        fprintf(thumb_outfile, "Total THUMB instructions: %d\n", thumb_insncount);
        fprintf(thumb_outfile, "THUMB Instructions executed: %d\n", thumb_executed);
        for (int i = 0; i < 1024; i++) {
            if (thumb_count[i])
                fprintf(thumb_outfile, "thumb%03X: %d\n", i, thumb_count[i]);
        }
    }
}
#endif
...
static int thumbExecute (void)
{
	...
	do {
		...
		PREFETCH_FIRST(&bus.busPrefetch);
...
#if (DIS_ENABLE)
		disThumb(oldArmNextPC, disMsg, DIS_VIEW_ADDRESS | DIS_VIEW_CODE);
		SLOG("%s", disMsg);
#endif
		bus.armNextPC = bus.reg[15].I;
		bus.reg[15].I += 2;
