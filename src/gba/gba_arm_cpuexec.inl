...
#else
#define OP_LDR    bus.reg[dest].I = rdMem32[addrpage](address)
#define OP_LDRH   bus.reg[dest].I = rdMem16[addrpage](address)
#define OP_LDRB   bus.reg[dest].I = rdMem8[addrpage](address)
#define OP_LDRSH  { u16 v = rdMem16[addrpage](address); if((address & 1)) v=(s8)v; bus.reg[dest].I = (s16)v; }
#define OP_LDRSB  bus.reg[dest].I = (s8)rdMem8[addrpage](address)
#endif
...
#define LDRSTR_INIT(CALC_OFFSET, CALC_ADDRESS) \
    ...
    CALC_OFFSET;                                        \
    const u32 address = CALC_ADDRESS;                         \
    const u8  addrpage = (address >> 24) & 0xF;
...
#define STR(CALC_OFFSET, CALC_ADDRESS, STORE_DATA, WRITEBACK1, WRITEBACK2, SIZE) \
    ...
    STORE_DATA;                                         \
    WRITEBACK2;                                         \
    int dataticks_val = DATATICKS_ACCESS_##SIZE##BIT(address);	\
    DATATICKS_ACCESS_BUS_PREFETCH_ARM(addrpage, dataticks_val); \
...
#define LDR(CALC_OFFSET, CALC_ADDRESS, LOAD_DATA, WRITEBACK, SIZE) \
    ...
    dataticks_value = DATATICKS_ACCESS_##SIZE##BIT(address); \
    DATATICKS_ACCESS_BUS_PREFETCH_ARM(addrpage, dataticks_value); \
...
#ifdef INSN_COUNTER
static void countstat(u32 opcode, int cond_res)
{
    ...
    if (!insncount) {
        ...
        char  fullPath[512];
        char  filename[] = "VBA-armcount.txt";
    ...
    if (outfile && insncount%1000000 == 0) {
        ...
        fprintf(outfile, "Memory Rd stats:\n");
        for (int i = 0; i < 16*8; i++) {
        	fprintf(outfile, "%4d: %d\n", i, memProfileCount[i]);
        }
...
#if (DIS_ENABLE)
		disArm(oldArmNextPC, disMsg, DIS_VIEW_ADDRESS | DIS_VIEW_CODE);
		SLOG("%s", disMsg);
#endif
		bus.armNextPC = bus.reg[15].I;
		bus.reg[15].I += 4;
